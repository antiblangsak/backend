...
class DWKController {
    public function getFamilyMembers($familyId, $registered) {
        ...
        foreach ($familyMembers as $familyMember) {
            $client = $familyMember->registeredAsClients->where('service_id', Constants::DWK_SERVICE_ID);
            if ($registered) {
                if ($client->count() > 0) {
                    $client = $client->first();
                    $data->push([
    ...
    public function getHistory($familyId) {
        ...
        $payments = Payment::where('service_id', Constants::DWK_SERVICE_ID)->get()
            ->whereIn('id', $paymentIds);

        $claims = Claim::where('service_id', Constants::DWK_SERVICE_ID)->get()
    ...
    public function getPaymentInfo($familyId) {
        $family = Family::find($familyId);
        $clientData = $family->familyMembersAsClients->where('service_id', Constants::DWK_SERVICE_ID);
